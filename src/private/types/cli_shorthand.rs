use nom::{
    branch::alt, bytes::complete::{escaped, is_not, tag, take_till}, character::complete::{alpha1, multispace0, one_of}, combinator::{eof, map}, error::Error, multi::many0, sequence::{delimited, preceded, separated_pair, terminated}, IResult
};
...
pub fn from_cli_string<'src>(input: &'src str) -> Result<Vec<(&'src str, CliShorthandValue<'src>)>, CliShorthandError> {
...
fn structure<'src>(input: &'src str) -> IResult<&str, Vec<(&'src str, CliShorthandValue<'src>)>> {
    many0(terminated(property, alt((tag(","), take_till(|c| c == '}'), eof))))(input)
}
...
fn simple_value<'src>(input: &'src str) -> IResult<&str, CliShorthandValue<'src>> {
    map(is_not(",}\n"), |s:&str|CliShorthandValue::SimpleValue(s.trim_ascii()))(input)
}
...
mod tests {
	...
	fn all_with_whitespace() {
		...
		assert_eq!(r1,r2,"with and without whitespace should parse to the same value");
	}
	...
	fn no_structs() {
		...
		if let [(k1,v1),(k2,v2)] = r.1.as_slice() {
			...
			assert!(matches!(*v1,CliShorthandValue::SimpleValue(v) if v == "string1"), "Expected CliShorthandValue::SimpleValue(string1): {v1:#?}");
			assert_eq!(*k2,"policyTemplateId");
			assert!(matches!(*v2,CliShorthandValue::SimpleValue(v) if v == "string2"), "Expected CliShorthandValue::SimpleValue(string2): {v2:#?}");
		} else {
			assert_eq!(r.1.len(),2);
		}
	}
		...
		if let [(k1,v1)] = r.1.as_slice() {
			...
			if let CliShorthandValue::Struct(s) = v1 {
				if let [(f1,f1v),(f2,f2v)] = s.as_slice() {
					...
					assert!(matches!(f1v, CliShorthandValue::SimpleValue("boolean")), "'unspecified' should have a simple value of 'boolean': {f1v:#?}");
					assert_eq!(*f2,"identifier");
					if let CliShorthandValue::Struct(s2) = f2v {
						if let [(f3,f3v),(f4,f4v)] = s2.as_slice() {
							assert_eq!(*f3,"entityType");
							assert!(matches!(f3v, CliShorthandValue::SimpleValue("string")), "'entityType' should have a simple value of 'string': {f3v:#?}");
