...
impl Read for GetSchema {
    ...
    async fn read(&self, policy_selector: Self::Input) -> Result<Self::Output, Self::Exception> {
        Ok(self.get_schema(&policy_selector.id().to_string()).await?)
    }
}
...
mod test {
    ...
    async fn get_schema_200() {
        let policy_selector = PolicySelector::from("ps-1".to_string());
        ...
        let result = schema_reader.read(policy_selector).await.unwrap();
    ...
    async fn get_schema_400() {
        ...
        let client = build_client(events);
        let schema_reader = GetSchema::new(client, BackoffStrategy::default());
        let result = schema_reader.read(policy_selector).await;
