...
async fn main() {
    ...
    let policy_set_provider = avp_local_agent::public::policy_set_provider::PolicySetProvider::from_client(policy_store_id, client).unwrap();
    let entity_provider = EntityProvider::default();

    let authorizer = Authorizer::new(AuthorizerConfigBuilder::default().policy_set_provider(Arc::new(policy_set_provider)).entity_provider(Arc::new(entity_provider)).build().unwrap());

    let response = authorizer.is_authorized(&construct_request(), &Entities::empty()).await.unwrap();
    println!("{response:?}")
}